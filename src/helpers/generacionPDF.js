...
export const generarPDFUso = async (filas, mes, nombreUsuario) => {
  ...
  const fechaInicioInforme = format(startOfMonth(mes), 'dd/MM/yyyy \'a las\' HH:mm')
  const fechaFinInforme = format(getMonth(new Date()) === getMonth(mes) ? new Date() : endOfMonth(mes), 'dd/MM/yyyy \'a las\' HH:mm')
  page.drawText(`Reporte de uso para ${mesInforme}`, { x: 210, y: 735, size: 12, font: helveticaBold })
  page.drawText(`Desde el ${fechaInicioInforme} hasta el ${fechaFinInforme}`, { x: 155, y: 715, size: 10, font: helvetica })
  ...
  page.drawText(`Reporte emitido el ${fechaEmision} a las ${horaEmision}.`, { x: 220, y: 110, font: helveticaBold, size: 8 })  
  page.drawText('Este reporte provee una estimación del uso de la plataforma y no es exhaustivo.', { x: 160, y: 90, font: helveticaItalic, size: 8 })  
  page.drawText('Refiérase a la factura y al resumen mensual que se entrega al final del período para conocer los valores oficiales.', { x: 100, y: 75, font: helveticaItalic, size: 8 })
