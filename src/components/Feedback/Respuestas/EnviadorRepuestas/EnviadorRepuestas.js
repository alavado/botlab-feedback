...
const EnviadorRepuestas = ({ idEncuesta }) => {
  ...
  const headersSinConsultaConfirmacion = [
    { name: 'FONO', type: 'text', display_name: 'FONO', required: true },
  ...
  const abrirDialogoArchivo = () =>
    document.getElementById('csv-enviador').click()
  const agregarFilaVacía = () =>
    setFilas([
      ...filas,
      {
        datos: [...headersSinConsultaConfirmacion].fill(''),
                  ...
                  {filas.length > 0 ? (
                    ...
                    <tr>
                      <td
                        ...
                        colSpan={data.data.data.length + 2}
                      >
